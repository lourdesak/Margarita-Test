...
void HistoManager::Book()
{
  ...
  G4int id = analysis->CreateH1("h1.1", "Stop: E_{kin} [MeV]", 200, -50., 50.);
  analysis->SetH1Activation(id, true);
  id = analysis->CreateH1("h1.2", "Stop: Z[cm]", 200, 0, 3.65);
  analysis->SetH1Activation(id, true);
  id = analysis->CreateH2("h2.1", "Stop: XY;x [cm];y [cm]", 120, -2., 7.5, 120, -2., 7.5);
  analysis->SetH2Activation(id, true);
  id = analysis->CreateH1("h1.3", "Stop: E_{kin}^{init} [MeV];counts", 200, 0., 100.);
  analysis->SetH1Activation(id, true);
  ...
  id = analysis->CreateH1("h1.5", "Incident: E_{kin}^{init} [MeV];counts", 200, 0., 100.);
  analysis->SetH1Activation(id, true);
